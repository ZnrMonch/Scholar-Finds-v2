...
<body class="bg-[url('resources/lib-bg.jpg')] font-nunito text-white flex">
    ...
    max-tablet:m-0 max-tablet:p-5 max-tablet:min-h-screen max-tablet:size-full max-tablet:rounded-none">
        ...
        <div class="flex-1 relative w-full flex flex-col gap-2.5 max-big-phone:gap-10">
            ...
            <div class="pl-70 pr-0 py-3 max-big-phone:p-0 max-big-phone:pt-37.5 w-full h-25 flex justify-between">
                <div class="h-full flex flex-col max-big-phone:items-center justify-end gap-1 **:leading-none">
                    ...
                    <i class="opacity-80 text-sm max-big-phone:text-xs">Joined in <span class="font-bold"><?php echo date("F d, Y", strtotime($dateJoined))?></span></i>
                </div>
                <div class="*:px-5 *:py-1 *:flex *:items-center *:gap-2 *:active:bg-neutral-300 text-off-white text-sm select-none">
                    <button onclick="logOut()" class="bg-red-900   cursor-pointer hover:opacity-80 active:scale-95 duration-200">
                        Sign out
                    </button>
                </div>
            </div>
            ...
            <div class="flex-1 w-full flex flex-col gap-2.5">
                <div class="text-lg font-bold flex items-center gap-5 select-none *:cursor-pointer *:duration-100">
                    <input type="radio" name="profile-info" id="about" class="peer/abt" hidden checked>
                    <input type="radio" name="profile-info" id="statistics" class="peer/stats" hidden>
                    <input type="radio" name="profile-info" id="settings" class="peer/sett" hidden>
                    <label for="about" class="opacity-40 peer-checked/abt:opacity-100">About</label>
                    <label for="statistics" class="opacity-40 peer-checked/stats:opacity-100">Statistics</label>
                    <label for="settings" class="opacity-40 peer-checked/sett:opacity-100">Settings</label>
                </div>
                <hr class="w-full border opacity-20">

                <!-- ABOUT -->
                <div class="flex-1 flex gap-20 hidden">
                    <div class="flex-1 text-justify">
                        <h2 class="py-2.5 font-bold text-lg">About Me</h2>
                        <p><?php echo $bio;?></p>
                    </div>
                    <div class="flex-1 flex flex-col">
                        <h2 class="py-2.5 font-bold text-lg">More Details</h2>
                        <div class="grid grid-cols-2 gap-5">
                            <span>
                                <h3 class="text-neutral-400 font-semibold text-xs select-none">COLLEGE</h3>
                                <p class="font-semibold"><?php echo $college == 'HSU' ? "Higher School ng UMak" : ($college == "CCIS" ? "College of Computing and Information Sciences" : $college);?></p>
                            </span>
                            <span>
                                <h3 class="text-neutral-400 font-semibold text-xs select-none">EMAIL ADDRESS</h3>
                                <a class="font-semibold text-blue-500 underline cursor-pointer active:cursor-text break-all" onclick="window.open('https://mail.google.com/mail/?view=cm&fs=1&to=<?php echo $email; ?>', '_blank')"><?php echo $email;?></a>
                            </span>
                            <span class="col-span-2">
                                <h3 class="text-neutral-400 font-semibold text-xs select-none">TAGS</h3>
                                <ul class="mt-1 font-semibold flex flex-wrap gap-2.5 text-sm *:px-4 *:py-1 *:rounded-full *:bg-neutral-200 *:border *:border-neutral-300 *:shadow-sm">
                                    <li>Full stack</li>
                                    <li>Front end</li>
                                </ul>
                            </span>
                ...
                <div class="flex-1 flex">
                    <div class="flex-1 p-2.5">
                        <div>
                            <h3 class="font-bold text-xl">Preferences</h3>
                            <form action="" class="select-none">
                                <span class="flex items-center gap-2.5">
                                    <input type="checkbox" name="notify-new" id="notify-new">
                                    <label for="notify-new" class="cursor-pointer" id="notify-new-label">Notify me when a new thesis is published.</label>
                                </span>
                                <span class="flex items-center gap-2.5">
                                    <input type="checkbox" name="notify-updates" id="notify-updates">
                                    <label for="notify-updates" class="cursor-pointer" id="notify-updates-label">Notify me about website updates.</label>
                                </span>
                                <span class="flex items-center gap-2.5">
                                    <input type="checkbox" name="notify-suggested" id="notify-suggested">
                                    <label for="notify-suggested" class="cursor-pointer" id="notify-suggested-label">Notify me when a suggested thesis is available.</label>
                                </span>
                                <button type="submit" class="mt-2.5 px-5 py-0.5 rounded-md enabled:bg-dgreen disabled:bg-neutral-400 text-off-white text-sm shadow-md select-none cursor-pointer enabled:hover:opacity-85 enabled:active:scale-95" id="save-preferences">Save</button>
                            </form>
                            <script>
                                const preferences = ['notify-new', 'notify-updates', 'notify-suggested'];
                                const saveButton = document.getElementById('save-preferences');
                                const initialStates = {};

                                preferences.forEach(id => {
                                    const checkbox = document.getElementById(id);
                                    initialStates[id] = checkbox.checked;

                                    checkbox.addEventListener('change', () => {
                                        const hasChanges = preferences.some(id => document.getElementById(id).checked !== initialStates[id]);
                                        saveButton.disabled = !hasChanges;

                                        const label = document.getElementById(id + '-label');
                                        label.classList.toggle('italic', checkbox.checked !== initialStates[id]);
                                    });
                                });
                    ...
                    <div class="w-100 p-2.5 flex flex-col gap-5">
                        <h2 class="font-bold text-xl text-red-800 flex items-center gap-2 select-none">
                        ...
                        <div class="flex-1">
                            <div class="flex flex-col gap-2.5">
                                <span>
                                    <h3 class="font-bold">Deactivate my account</h3>
                                    <p class="text-sm opacity-80">You can reactivate your account anytime.</p>
                                </span>
                                <span>
                                    <h3 class="font-bold text-red-800">Delete my account</h3>
                                    <p class="text-sm opacity-80">This action is irreversible. All your data will be lost.</p>
                                </span>
                            </div>
                        </div>
                    </div>
